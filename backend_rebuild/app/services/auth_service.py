...
class AuthService:
    ...
    async def register(
        ...
        *,
        ...
        try:
            # Generate the user's UUID before creating
            # related records.
            await self.session.flush()

            # Every new HomeLink account gets its three
            # posting-credit balances once during creation.
            await CreditService(
                self.session,
            ).initialize_balances(
                user_id=user.id,
            ...
            # User + credit balances + auth session are
            # committed together.
    ...
    async def login(
        ...
        *,
        ...
        user = (
            await self.users.get_by_email_for_update(
                str(
                    payload.email,
                )
            )
    ...
    async def refresh(
        ...
        *,
        ...
        try:
            session_id = parse_compound_token(
                refresh_token,
            )
        ...
        auth_session = (
            await self.auth_sessions.get_by_id(
                session_id,
            )
        )
    ...
    def _record_failed_login(
        *,
        ...
        if (
            ...
            >= settings.force_reset_after_failures
        ):
